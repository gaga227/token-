...
export const channelFormSchema = z
  .object({
    ...
    // 异步任务：创建时返回上游原始 task id（需用户侧同时开启）
    return_upstream_task_id: z.boolean().optional(),
...
export function transformChannelToFormDefaults(
  ...
  let returnUpstreamTaskId = false
  ...
  if (channel.settings) {
    try {
      ...
      returnUpstreamTaskId = parsed.return_upstream_task_id === true
  ...
  return {
    ...
    return_upstream_task_id: returnUpstreamTaskId,
...
function buildSettingsJSON(formData: ChannelFormValues): string {
  ...
  settingsObj.return_upstream_task_id =
    formData.return_upstream_task_id === true
