...
> 相关文档：[视频生成 API 文档](cn-video-generation-api.md)（任务创建/查询、`asset://` 引用、参数说明）
...
1. **Base URL**：你的 灵枢AI 网关地址，例如 `https://token.nexuscore.net.cn`。下文以 `{$BASE}` 代指。
...
```json
{
  ...
  "message": "",
  "data": {
    "Url": "https://token.nexuscore.net.cn/api/asset/files/ab/abcd1234....mp4",
    "AssetType": "Video",
    "Size": 10485760
  }
}
...
{
  "GroupId": "group-na-...",
  "URL": "https://token.nexuscore.net.cn/api/asset/files/ab/abcd....mp4",
  "AssetType": "Video",
  "Name": "产品演示片段",
  "ProjectName": "default"
}
...
- `URL` 必须是 http/https 地址，≤ 8192 字节，不允许内嵌凭据。
  - 指向本网关上传接口返回的地址（`/api/asset/files/...`）时直接复用已存储的文件。
  - 指向其他外部地址时，网关会**先下载该文件并转存到网关存储**，再以网关自己的公网地址同步给上游渠道。若下载失败（如地址仅调用方内网可达、防盗链、已失效），返回 HTTP 400 `InvalidParameter.URL`，并提示改用 `POST /api/asset/upload` 上传本地文件。
  - 转存同样受文件大小与格式限制（见上文「文件限制」），且实际文件类型需与 `AssetType` 一致，否则返回 HTTP 400。
...
示例（视频生成请求，注意：`asset://` 引用**仅在原生端点** `POST /api/v3/contents/generations/tasks` 支持，详见 [视频生成 API 文档](cn-video-generation-api.md)）：
...
```bash
BASE=https://token.nexuscore.net.cn
TOKEN=sk-xxxxxxxx
