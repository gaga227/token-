...
面向下游接入方（应用开发者 / 企业内部系统）。本文档覆盖 **MiniMax-H3 系列文生视频 / 图生视频 / 多参参考视频任务**的创建、查询与计费规则。该系列经平台网关（灵枢AI sora 任务通道）转发至 maitoken 上游，素材字段采用 **MiniMax 原生文档风格**，网关原样透传并同步完成素材计费识别。
...
4. 金额为**任务最终扣费**（任务失败已自动全额退款，此时本组字段为 0）；纯文生（T2V）任务输入为 0、全部归输出。
...
1. **预扣 + 失败全额退款**（灵枢AI 任务机制）：创建任务时按上述规则预扣，任务终态失败时**自动全额退款**（超时任务同样退款）。提交前仍建议确认素材 URL 可达、参数合法，避免浪费等待时间。
2. **计费档位看 `size` 字段**：若按 maitoken 文档习惯只传 `resolution: "2K"` 而不传 `size`，将按 768P（缺省 `720x1280`）计费——传 2K 输出时请务必同时传 `size: "1440x2560"`。
3. 视频参考素材 URL 需大陆可达，境外直链会导致素材下载失败（任务失败会全额退款，但会浪费生成等待时间）。
...
```bash
# 1. 创建任务
curl -X POST https://token.nexuscore.net.cn/v1/videos \
...
curl https://token.nexuscore.net.cn/v1/videos/task_xxxx -H "Authorization: Bearer sk-xxxx"
