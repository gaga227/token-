...
如需在生成任务中引用素材库里的图片 / 视频 / 音频（`asset://` 引用），请先阅读 [素材库 API 接入文档](cn-asset-library-api.md)。
...
| 项目 | 说明 |
|------|------|
| Base URL | 平台网关地址，如 `https://token.nexuscore.net.cn` |
...
完整素材上传 / 同步 / 状态查询流程见 [素材库 API 接入文档](cn-asset-library-api.md)。
...
前置步骤（上传素材并等 `Active`）见 [素材库 API 接入文档](cn-asset-library-api.md) 的端到端示例。
